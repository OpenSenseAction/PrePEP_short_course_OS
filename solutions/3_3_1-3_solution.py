...
for pws_id in  ['ams74', 'ams134', 'ams113', 'ams36', str(ds_pws.isel(id=(ds_pws.hi_flag>0).sum(dim='time').argmax()).id.values)]:
    ...
    ds_pws.fz_flag.where(ds_pws.fz_flag>0).sel(id=pws_id).plot.line(x='time',lw=1,color='pink',label='FZ',ax=ax[0])
    ds_pws.hi_flag.where(ds_pws.hi_flag>0).sel(id=pws_id).plot.line(x='time',lw=1,color='cyan',label='HI',ax=ax[0])
    ax[0].legend()
